...
欢迎学习Node.js。Node.js是一个编写网络和Web应用的激动人心的新平台！过去几年，关于它总是“绯闻”不断，并且在开发者社区聚集了大量的人气！本书中，我将会教会你关于它的更多内容，为何它如此特别让开发者钟情于它，并让你可以在短期内上手Node.js程序。你将会看到开发者更喜欢为你起一个简洁的名字，Node或者"node"。当然，我尽最大的努力介绍它！


##　Why Node.js?(为何是Node.js?)
Node.js has arisen for a couple of primary reasons, which I explain next.
一系列主要原因促使Node.js的兴起, 下文将会解释.
...
过去，编写Web应用程序是一个非常标准的过程。你机器上有一个或多个服务器侦听端口 (例如，http的80端口)，当服务器收到一个请求时，您的机器上会 forks(分叉) 新的 process(进程) 或 thread(线程)，开始处理查询并和响应查询。
...
这项工作经常涉及与外部服务通信，如数据库，内存缓存，外部计算服务器，甚至只是文件系统。
...
1. 每一个线程或进程都携带着一些开销。在某些PHP + Apache机器上，每个进程携带者多达10–15MB的开销。即使是无休止运行的大型服务器环境中，为处理请求进行fork线程操作，每个线程携带者创建一个新的堆栈和执行环境的开销，你将会经常遇到服务器可用内存的限制。
...
当外部组件（组件）最终返回响应时，进程或线程的“冻结”状态被解除，然后向客户端发送响应，接着再准备接踵而来的的请求。
...
尽管这样的模型易于理解并跟它"打交道"工作，当你的脚本花费大量的时间等待数据库服务器完成运行查询的响应，这是非常低效的！对于很多现代Web应用一种极为常见的场景。
...
JavaScript，这门旧的(1995年左右被创建)的编写客户端脚本的编写语言，它人气很高，却经常被辱骂，真让人抱打不平，可喜的是，已经越来越流行了。现代Web浏览器正在清理它那些糟糕的API实现,并为它带来新的功能，使它的内心变得更强大，而不至于让人感觉它是一个另类的存在。随着为浏览器量身定制的客户端库出现，如jQuery，Script.aculo.us或Prototype库，JavaScript的编程变得有趣、高出。笨重的API已被清理干净，相反的是，却增加了乐趣和动态效果。
...
正因为这些关键因素的存在，开发者有了一个创建网络(web)应用开发的新方法。因此，Node.js的诞生。
...
这样的性能速度和能力给我们带来一些“警示”，但是，在你开始Node编程前必须瞪大眼睛充分解Node的原理。
...
这个编程模型的另一个局限性是: 它真的是以那些需要很多不同的进程、服务器和服务做很多事情，作为应用程序的中心。
